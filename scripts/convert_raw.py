...
def convert(job):
    ...
    stat = p.stat()
    key = {'source': str(p), 'bytes': stat.st_size, 'mtime_ns': stat.st_mtime_ns,
    ...
    with rawpy.imread(str(p)) as raw:
        pixels = raw.postprocess(use_camera_wb=True, output_color=rawpy.ColorSpace.sRGB,
                                output_bps=8, demosaic_algorithm=rawpy.DemosaicAlgorithm.AHD)
    ...
    record = {'input': key, 'jpg_bytes': dest.stat().st_size, 'width': image.width, 'height': image.height}
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as pool:
            ...
            for n, future in enumerate(as_completed(futures), 1):
                ...
                if n % 25 == 0 or n == len(items):
                    print(f'已转换 {n}/{len(items)}；耗时 {time.monotonic()-start:.0f} 秒；{result["name"]}', flush=True)
        (output / 'conversion-errors.json').write_text(json.dumps(failures, ensure_ascii=False, indent=2), encoding='utf-8')
