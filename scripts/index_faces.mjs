...
const human = new Human({
  ...faceConfig(pathToFileURL(modelPath + '/').href, 'wasm'),
  wasmPath:
    path.resolve('node_modules/@tensorflow/tfjs-backend-wasm/dist').replaceAll('\\', '/') + '/',
...
const cache = new Map(
  (previous.pipeline === 'overlap-tiles-v2' ? previous.photos : []).map((p) => [p.id, p]),
);
const results = [];
...
for (const [index, photo] of photos.entries()) {
  ...
  if (cache.get(photo.id)?.preview === photo.preview) results.push(cache.get(photo.id));
    ...
    if (Math.max(metadata.width, metadata.height) > 1000) {
      const w = Math.min(metadata.width, Math.ceil(metadata.width * 0.62)),
        h = Math.min(metadata.height, Math.ceil(metadata.height * 0.62));
      for (const left of [0, metadata.width - w])
        for (const top of [0, metadata.height - h])
          windows.push({ left, top, width: w, height: h });
    }
    const faces = [];
    for (const window of windows) {
      ...
      const found = await human.detect(tensor);
      tensor.dispose();
      if (found.error) throw new Error(found.error);
      for (const f of found.face.filter((f) => f.embedding?.length === 1024 && f.boxScore >= 0.6)) {
        const scale = window.width / info.width;
        const box = [
          f.box[0] * scale + window.left,
          f.box[1] * scale + window.top,
          f.box[2] * scale,
          f.box[3] * scale,
        ];
        const duplicate = faces.some((previous) => {
          const a = previous.box;
          const intersection =
            Math.max(0, Math.min(a[0] + a[2], box[0] + box[2]) - Math.max(a[0], box[0])) *
            Math.max(0, Math.min(a[1] + a[3], box[1] + box[3]) - Math.max(a[1], box[1]));
          return intersection / Math.min(a[2] * a[3], box[2] * box[3]) > 0.5;
        });
  ...
  if ((index + 1) % 10 === 0 || index + 1 === photos.length) {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        version: 1,
        pipeline: 'overlap-tiles-v2',
        model: 'human-3.3.6-faceres',
        photos: results,
      }),
    );
    fs.renameSync(output + '.tmp', output);
    console.log(
      `人脸索引 ${index + 1}/${photos.length}；累计 ${results.reduce((n, p) => n + p.faces.length, 0)} 张脸`,
    );
  }
}
