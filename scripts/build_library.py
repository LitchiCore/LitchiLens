...
def build(source, exports, generated, output, center=False, workers=4):
    ...
    try:
        ...
        shutil.copyfile(ROOT / "THIRD_PARTY.md", vendor / "THIRD-PARTY.md")
        ...
        if sponsor.is_file():
            ...
            atomic_bytes(
                html, html.read_bytes().replace(b'data-support="off"', b'data-support="on"')
            )
        else:
            (output / "sponsor.png").unlink(missing_ok=True)
        catalog = {
            "version": 2,
            "title": "军训影集",
            "updated": datetime.now(timezone.utc).isoformat(),
            "dates": sorted({p["date"] for p in photos}),
            "photos": photos,
        }
        atomic_bytes(
            output / "catalog.json",
        ...
        report = dict(
            ...
            missing_jpg=missing,
            ids=len(photos),
            versions=sum(len(p["versions"]) for p in photos),
        )
        atomic_bytes(
            output.parent / (output.name + "-report.json"),
            json.dumps(report, ensure_ascii=False, indent=2).encode(),
        )
        print(
            f"构建完成：{len(photos)} 个 ID，{report['versions']} 个 JPG 版本；缺少 JPG：{len(missing)}",
            flush=True,
        )
        return catalog
    finally:
