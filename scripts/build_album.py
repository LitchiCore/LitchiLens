...
def pair_nef(jpg, day, raw_index):
    ...
    exact = [p for p in candidates if p.parent == jpg.parent and p.stem.casefold() == jpg.stem.casefold()]
    if len(exact) == 1:
        return exact[0], None
    if len(candidates) == 1:
...
def build(args):
    ...
    try:
        ...
        raise ValueError('此输出目录正在构建；若上次中断，请确认无构建进程后移除 .build.lock。') from exc
    os.close(fd)
    try:
...
def build_locked(args, source, output):
    ...
    detector = FaceDetector() if images and not args.inventory_only and not args.center_crop else None
    ...
    for number, (day, path) in enumerate(sorted(images), 1):
        ...
        identity = hashlib.sha256((relative + digest + PIPELINE + str(args.center_crop)).encode()).hexdigest()[:24]
        ...
        record = {'id': identity, 'name': path.name, 'date': day, 'width': width, 'height': height,
                  'thumb': thumb_path, 'preview': preview_path, 'jpg': {'url': jpg_url, 'bytes': before.st_size}, 'nef': None}
        if raw and args.include_nef:
            raw_stat = raw.stat()
            raw_tag = hashlib.sha256(f'{raw.relative_to(source)}:{raw_stat.st_size}:{raw_stat.st_mtime_ns}'.encode()).hexdigest()[:16]
            raw_url = f'downloads/{identity}-{raw_tag}.nef'
            atomic_copy(raw, output / raw_url)
            if (raw.stat().st_size, raw.stat().st_mtime_ns) != (raw_stat.st_size, raw_stat.st_mtime_ns):
    ...
    catalog = {'version': 1, 'title': args.title, 'updated': datetime.now(timezone.utc).isoformat(),
               'dates': sorted(dates), 'photos': photos}
    report = {'jpg_found': len(images), 'nef_found': sum(map(len, raw_index.values())), 'published': len(photos),
              'thumbnail_modes': dict(modes), 'warnings': warnings, 'inventory_only': args.inventory_only}
...
def main():
    ...
    parser.add_argument('--inventory-only', action='store_true', help='只生成等待照片页面，不发布任何照片')
    parser.add_argument('--include-nef', action='store_true', help='复制配对成功的 NEF，开启原片下载')
    parser.add_argument('--center-crop', action='store_true', help='停用人脸检测，统一居中裁剪')
    parser.add_argument('--skip-invalid', action='store_true', help='显式跳过损坏或扩展名不符的 JPG，并记录报告')
