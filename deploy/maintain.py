...
def current_ip():
    ...
    if not routes:
        raise RuntimeError("没有 IPv6 默认路由")
    ...
    for interface in addresses:
        for item in interface.get("addr_info", []):
            ...
            if (
                ...
                and item.get("preferred_life_time") != 0
            ):
                candidates.append(str(address))
    if len(candidates) != 1:
        raise RuntimeError("无法唯一确定稳定公网 IPv6")
    return candidates[0]
            ...
            with socket.create_connection(("::1", port), timeout=3) as raw:
                with ssl.create_default_context().wrap_socket(raw, server_hostname=ip) as conn:
                    ...
                    with conn.makefile("rb") as response:
                        status = response.readline(4096).decode().strip()
                    if len(status.split()) < 2 or status.split()[1] != "200":
                        raise RuntimeError(f"页面检查失败：{status}")
                    return
...
def main():
    with open("/run/litchilens-maintain.lock", "w") as lock:
        ...
        ip = current_ip()
        command = [
            shutil.which("certbot") or "/snap/bin/certbot",
            "certonly",
            "--webroot",
            "--webroot-path",
            "/var/lib/letsencrypt",
        ...
        if config.get("ipv6") and config["ipv6"] != ip:
