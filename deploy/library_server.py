...
class Library:
    def __init__(self, site, state, faces=None, completion_log=None):
        ...
        for photo in self.catalog["photos"]:
            identity = photo["id"]
            if (
                not re.fullmatch(r"\d{4}-\d{2}-\d{2}_[A-Z0-9_-]+", identity)
                or identity in self.photos
            ):
        ...
        self.log_offset = (
            self.completion_log.stat().st_size
        ...
        if faces and Path(faces).exists():
            ...
            for photo in data["photos"]:
                if (
                    photo["id"] in self.photos
                    and photo["preview"] == self.photos[photo["id"]]["preview"]
                ):
    ...
    def refresh_revision(self):
        self.revision = (
            str(self.catalog.get("updated", ""))
            + ":"
            + hashlib.sha256(json.dumps(sorted(self.state["hidden"])).encode()).hexdigest()[:12]
        )

    def traffic(self, visitor=None):
        now = time.time()
        self.visitors = {k: v for k, v in self.visitors.items() if v > now - 90}
        if visitor and re.fullmatch(r"[a-f0-9-]{36}", visitor) and len(self.visitors) < 5000:
            self.visitors[visitor] = now
        self.downloads = {k: v for k, v in self.downloads.items() if v > now - 10800}
        if self.completion_log and self.completion_log.exists():
            if self.completion_log.stat().st_size < self.log_offset:
                self.log_offset = 0
            with self.completion_log.open() as handle:
                handle.seek(self.log_offset)
                for line in handle:
                    try:
                        self.downloads.pop(json.loads(line)["id"], None)
    ...
    def ids(self, data):
        ...
        if (
            ...
            or not all(isinstance(i, str) and i in self.photos for i in ids)
        ):
    ...
    def selected_files(self, ids, kind):
        ...
        for identity in ids:
            ...
            if kind == "nef":
                file = p.get("nef")
                if file:
                    result.append((identity, file["url"], identity + ".NEF"))
            else:
                variants = (
                    p["versions"]
                    if kind == "jpg"
                    else [v for v in p["versions"] if v["label"] == labels[kind]]
                )
                if variants:
                    v = variants[0]
        ...
        def do_GET(self, head=False):
            ...
            if route == "catalog.json":
                with library.lock:
                    data = dict(
                        ...
                        photos=[p for p in library.catalog["photos"] if library.visible(p["id"])],
                    )
            ...
            with library.lock:
                if route == "api/status":
                    return self.reply(
                        200,
                        {
                            "faces_ready": bool(library.faces),
                            "face_photos": len({i for i, _ in library.faces if library.visible(i)}),
                        },
                        head,
                    )
                ...
                if route.startswith(("media/", "downloads/")):
                    ...
                    if not any(library.visible(i) for i in owners):
                        return self.reply(404, {"error": "照片不存在或已隐藏"}, head)
                    if route.startswith("downloads/") and not head:
                        request_id = self.headers.get("X-Request-Id", "")
                        if re.fullmatch(r"[a-f0-9]{32}", request_id):
                            library.downloads[request_id] = time.time()
                ...
                if route.startswith("api/download/"):
                    ...
                    if not ticket or ticket["expires"] < time.time():
                        return self.reply(410, {"error": "下载链接已过期，请重新选择"}, head)
                    files = library.selected_files(ticket["ids"], ticket["kind"])
                    if not files:
                        return self.reply(410, {"error": "所选照片已隐藏或没有此版本"}, head)
                else:
            ...
            try:
                with zipfile.ZipFile(
                    self.wfile, "w", compression=zipfile.ZIP_STORED, allowZip64=True
                ) as archive:
                    for identity, asset, name in files:
                        with library.lock:
                            if not library.visible(identity):
                                continue
                ...
                with library.lock:
                    library.downloads.pop(request_id, None)

        def do_POST(self):
            # JSON + 自定义请求头要求浏览器同源；不提供 CORS。
            origin = self.headers.get("Origin")
            host = self.headers.get("X-Forwarded-Host", self.headers.get("Host"))
            if self.headers.get("X-LitchiLens") != "1" or (
                origin and origin not in ("https://" + host, "http://" + host)
            ):
            ...
            try:
                ...
                if route == "/api/search":
                    ...
                    if (
                        ...
                        or len(vector) != 1024
                        or not all(
                            type(x) in (float, int) and math.isfinite(x) and abs(x) < 100
                            for x in vector
                        )
                    ):
                        raise ValueError("人脸特征无效，请重新选择自拍")
                    if not library.faces:
                        return self.reply(503, {"error": "人脸索引正在准备，请先浏览照片库"})
                    scores = {}
                    with library.lock:
                    ...
                    with library.lock:
                        matches = [
                            {"id": i, "score": round(s, 3)}
                            for i, s in sorted(scores.items(), key=lambda x: -x[1])
                            if library.visible(i)
                        ][:100]
                    return self.reply(
                        200, {"matches": matches, "notice": "相似度只用于排序，不代表身份概率"}
                    )
                with library.lock:
                    ...
                    if route == "/api/hide":
                        ...
                        for identity in ids:
                            library.state["hidden"][identity] = now
                        try:
                            library.persist()
                        except Exception:
                    ...
                    if route == "/api/recolor":
                        ...
                        if not isinstance(reason, str) or not 1 <= len(reason.strip()) <= 500:
                            raise ValueError("请填写 1 至 500 字调色需求")
                        ids = [i for i in ids if library.visible(i)]
                        if not ids:
                            raise ValueError("所选照片已隐藏")
                        record = {
                    ...
                    if route == "/api/download":
                        ...
                        if kind not in ("jpg", "nef", "standard", "edited", "retouched"):
                            raise ValueError("下载版本无效")
                        files = library.selected_files(ids, kind)
                        if not files:
                            raise ValueError("所选照片没有此版本，或已经隐藏")
                        if len(files) <= 10:
                            return self.reply(
                                200,
                                {
                                    ...
                                    "files": [
                                        {"id": identity, "url": asset, "name": name}
                                        for identity, asset, name in files
                                    ],
                                },
                            )
                        library.tickets = {
                            k: v for k, v in library.tickets.items() if v["expires"] > time.time()
                        }
                        if len(library.tickets) > 2000:
                            return self.reply(429, {"error": "下载繁忙，请稍后重试"})
                        token = secrets.token_urlsafe(24)
                        library.tickets[token] = {
                            "ids": ids,
                            "kind": kind,
...
if __name__ == "__main__":
    ...
    print(
        f"LitchiLens API 已启动：{len(library.photos)} 个 ID，{len(library.faces)} 张脸", flush=True
    )
