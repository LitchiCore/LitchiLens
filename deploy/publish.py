...
def stage(site, release, token):
    ...
    if catalog.get("version") != 1 or not isinstance(catalog.get("photos"), list):
        raise ValueError("相册索引无效")
    assets = set()
    for photo in catalog["photos"]:
        for value in (
            ...
            photo["preview"],
            photo["jpg"]["url"],
            photo.get("nef", {}).get("url") if photo.get("nef") else None,
        ):
            if value is None:
                continue
            if not re.fullmatch(r"(media|downloads)/[a-f0-9-]+\.(jpg|nef)", value):
                raise ValueError("相册资产路径无效")
            path = site / value
            if (
                not path.is_file()
                or path.is_symlink()
                or not path.resolve().is_relative_to(site.resolve())
            ):
                raise ValueError("照片文件不存在或越界")
            assets.add(value)
