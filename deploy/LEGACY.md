...
以下是第一版部署方式，仅供旧站点维护参考。第二版不使用这里的逐连接限速或固定 NEF 分包。新安装请从 [当前部署说明](README.md) 开始。
...
NEF ZIP 单独保存在 `/srv/litchilens/bundles/批次编号/`，不进入 Git，也不混入相册发布目录。`/srv/litchilens/bundles/packages.json` 是下载页清单，字段为 `version: 1` 和 `packages` 数组；每条包含 `number`、`name`、`url`、`count`、`bytes`、`dates`、`sha256`。URL 使用 `bundles/批次编号/nef-包号.zip`。先完成上传并核对双端 SHA-256，再原子替换清单；不要把未完成文件列入清单。

端口下的 `/nef` 自动进入分包页。ZIP 每 IP 最多 2 个并发，本站该区最多 4 个并发，单连接上限 6 MiB/s，实际总速度仍受家庭上行带宽限制。支持单范围请求续传。以下较低限速只用于单张照片下载。
