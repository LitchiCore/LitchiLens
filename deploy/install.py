...
def main():
    ...
    if not 1024 <= args.port <= 65535:
        raise ValueError("端口应在 1024 至 65535 之间")
    ...
    if exists and not args.resume:
        raise RuntimeError(
            "已有安装，拒绝覆盖。更新使用 publish.py；恢复中断的首次安装使用 --resume。"
        )
    if not args.resume and re.search(rf":{args.port}\s", run(["ss", "-lnt"])):
        raise RuntimeError("端口已被占用")
    spec = importlib.util.spec_from_file_location(
        "maintain", Path(__file__).with_name("maintain.py")
    )
    ...
    if args.resume:
        ...
        if previous["port"] != args.port or not re.fullmatch("[a-f0-9]{32}", previous["token"]):
            raise ValueError("恢复配置不匹配")
        if (
            not Path("/srv/litchilens/current")
            .resolve(strict=True)
            .is_relative_to("/srv/litchilens/releases")
        ):
            raise ValueError("发布目录不匹配")
    ...
    config_dir = Path("/etc/litchilens")
    config_dir.mkdir(mode=0o700, exist_ok=args.resume)
    config = config_dir / "site.json"
    config.write_text(json.dumps({"port": args.port, "token": token, "ipv6": ip}))
    config.chmod(0o600)
    root = Path("/srv/litchilens")
...
""")
    ...
    try:
        ...
        run(["systemctl", "reload", "nginx"])
        raise
    helper = Path("/usr/local/sbin/litchilens-maintain")
    shutil.copyfile(Path(__file__).with_name("maintain.py"), helper)
    helper.chmod(0o755)
...
""")
    ...
    run(["systemctl", "enable", "--now", "litchilens-maintain.timer"])
