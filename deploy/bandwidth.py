...
def apply(config, stop=False):
    ...
    roots = [
        q for q in json.loads(run("tc", "-j", "qdisc", "show", "dev", interface)) if q.get("root")
    ]
    ...
    default_fq = (
        len(roots) == 1
        and roots[0]["kind"] == "fq_codel"
        and options.get("limit") == 10240
        and options.get("flows") == 1024
        and options.get("quantum") == 1514
        and options.get("target") in (4999, 5000)
        and options.get("interval") in (99999, 100000)
        and options.get("memory_limit") == 33554432
        and options.get("ecn") is True
    )
