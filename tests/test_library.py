...
class LibraryTest(unittest.TestCase):
    def test_source_dates_and_ambiguity(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            recycle = source / "回收站"
            recycle.mkdir()
            (recycle / "DSC_0002.NEF").write_bytes(b"excluded")
            exports = root / "exports" / "wrong-date"
            ...
            self.assertEqual(records[0]["date"], "2026-08-26")
            self.assertEqual(len(records[0]["versions"]), 1)
            self.assertEqual(len(warnings["unmatched_jpg"]), 1)
            other = source / "2026-8-27"
            other.mkdir()
            (other / "DSC_0001.NEF").write_bytes(b"raw2")
            records, warnings = collect(source, exports.parent)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for n, label in enumerate(("精调", "标准转换")):
                ...
                for suffix in ("-0.jpg", "-1.jpg"):
                    Image.new("RGB", (32, 32)).save(site / "media" / (key + suffix))
                Image.new("RGB", (32, 32)).save(site / "downloads" / (key + ".jpg"))
                versions.append(
                    {
                        "id": key,
                        "label": label,
                        "thumb": f"media/{key}-0.jpg",
                        "preview": f"media/{key}-1.jpg",
                        "jpg": {
                            "url": f"downloads/{key}.jpg",
                            "bytes": (site / "downloads" / (key + ".jpg")).stat().st_size,
                        },
                    }
                )
            raw = site / "downloads" / ("a" * 64 + ".nef")
            raw.write_bytes(b"raw-file")
            photo = {
                "id": "2026-08-26_DSC_0001",
                "name": "DSC_0001",
                "date": "2026-08-26",
                "versions": versions,
                "nef": {"url": "downloads/" + raw.name, "bytes": 8},
                "preview": versions[0]["preview"],
            }
            catalog = {"version": 2, "photos": [photo], "dates": ["2026-08-26"]}
            (site / "catalog.json").write_text(json.dumps(catalog))
            state = root / "state.json"
            library = Library(site, state)
            ...
            def request(route, data=None, headers=None):
                req = Request(
                    ...
                    headers=headers or {"Content-Type": "application/json", "X-LitchiLens": "1"},
                )
                try:
            ...
            try:
                ids = [photo["id"]]
                status, payload, _ = request("api/download", {"ids": ids, "kind": "standard"})
                self.assertEqual(status, 200)
                result = json.loads(payload)
                self.assertEqual(result["mode"], "files")
                ticket = result["files"][0]["url"]
                status, payload, headers = request(ticket)
                self.assertEqual(status, 200)
                ...
                with ThreadPoolExecutor(max_workers=8) as pool:
                    statuses = list(
                        pool.map(
                            lambda n: request(
                                "api/recolor", {"ids": ids, "reason": f"并发留言 {n}"}
                            )[0],
                            range(8),
                        )
                    )
                ...
                self.assertEqual(
                    request("api/hide", {"ids": ["invalid"], "confirmed": True})[0], 400
                )
                self.assertEqual(request("api/search", {"embedding": [0] * 1023})[0], 400)
                self.assertEqual(
                    request(
                        "api/hide",
                        {"ids": ids, "confirmed": True},
                        {
                ...
                for version in versions:
                    for asset in (version["thumb"], version["preview"], version["jpg"]["url"]):
                        self.assertEqual(request(asset)[0], 404)
    ...
    def test_download_file_threshold(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for n in range(11):
                key = f"{n:064x}"
                jpg = f"downloads/{key}.jpg"
                preview = f"media/{key}-0.jpg"
                Image.new("RGB", (8, 8)).save(root / jpg)
                Image.new("RGB", (8, 8)).save(root / preview)
                raw = f"downloads/{key}.nef"
                (root / raw).write_bytes(b"raw")
                photos.append(
                    {
                        "id": f"2026-08-26_DSC_{n:04d}",
                        "versions": [
                            {
                                "label": "标准转换",
                                "jpg": {"url": jpg},
            ...
            def prepare(count, kind="jpg"):
                req = Request(
                    base + "api/download",
                    data=json.dumps(
                        {"ids": [p["id"] for p in photos[:count]], "kind": kind}
                    ).encode(),
            ...
            try:
                ...
                with zipfile.ZipFile(io.BytesIO(payload)) as archive:
                    ...
                    self.assertEqual(
                        archive.read(photos[1]["id"] + ".jpg"),
                        (root / photos[1]["versions"][0]["jpg"]["url"]).read_bytes(),
                    )
            finally:
                server.shutdown()
                server.server_close()
    ...
    def test_traffic_cleanup(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "catalog.json").write_text('{"version":2,"photos":[]}')
            ...
            self.assertEqual(status["visitors"], 1)
            for _ in range(5):
                self.assertEqual(
                    library.traffic("12345678-1234-1234-1234-123456789abc")["visitors"], 1
                )
